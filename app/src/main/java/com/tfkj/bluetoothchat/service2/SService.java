...
public class SService {
     ...
     */
    public SService(Context context, Handler handler) {
        mContext = context;
    ...
    public void write(String clientDeviceAddress, byte[] out) {
        ...
        if (r != null) {
            r.write(out, true);
            mHandler.obtainMessage(Constants.SERVICE_MESSAGE_WRITE, -1, -1, out)
                            .sendToTarget();
        } else {
            ...
            while (mState == Constants.STATE_CONNECTED) {
                try {
                    ...
                    if(!TextUtils.isEmpty(result)){
                        MessageBean message = JSON.parseObject(result, MessageBean.class);
                        ...
                        mHandler.obtainMessage(Constants.SERVICE_MESSAGE_READ, newBuffer.length, -1, newBuffer)
                                .sendToTarget();
                    }
                } catch (IOException e) {
...
//                if (isUpdateUI) {
//                    // Share the sent message back to the UI Activity
//                    mHandler.obtainMessage(Constants.SERVICE_MESSAGE_WRITE, -1, -1, buffer)
//                            .sendToTarget();
//                }
            } catch (IOException e) {
